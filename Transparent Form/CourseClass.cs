...
namespace Transparent_Form
{
    class CourseClass
    {
        ...
        public bool insetCourse(string cName, int hr, string tn)
        {
            MySqlCommand command = new MySqlCommand("INSERT INTO `course`(`CourseName`, `CourseHour`, `Tutor_Name`) VALUES (@cn,@ch,@tn)", connect.getconnection);
            //@cn,@ch,@tn
            command.Parameters.Add("@cn", MySqlDbType.VarChar).Value = cName;
            command.Parameters.Add("@ch", MySqlDbType.Int32).Value = hr;
            command.Parameters.Add("@tn", MySqlDbType.VarChar).Value = tn;
            connect.openConnect();
        ...
        {
            MySqlCommand command = new MySqlCommand("UPDATE `course` SET`CourseName`=@cn,`CourseHour`=@ch,`Tutor_Name`=@tn WHERE  `CourseId`=@id", connect.getconnection);
            //@id,@cn,@ch,@desc
